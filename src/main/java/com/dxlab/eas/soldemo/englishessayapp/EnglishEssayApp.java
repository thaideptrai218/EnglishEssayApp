...
class StudentFrame extends JFrame {
    ...
    private void loadTopics() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(EnglishEssayApp.TOPICS_FILE))) {
            ...
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(EnglishEssayApp.TOPICS_FILE))) {
                writer.write("T1 | Some people think that technology improves education. To what extent do you agree or disagree?");
                writer.newLine();
                ...
                writer.newLine();
                topicTableModel.addRow(new Object[]{"T1", "Some people think that technology improves education..."});
                topicTableModel.addRow(new Object[]{"T2", "Discuss the advantages and disadvantages of living in a big city."});
                topicTableModel.addRow(new Object[]{"T3", "Should governments invest more in public transportation..."});
            } catch (IOException e) {
    ...
    private void saveDraft() {
        ...
        if (!content.isEmpty()) {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(EnglishEssayApp.DRAFTS_FILE, true))) {
                ...
                writer.newLine();
                draftTableModel.addRow(new Object[]{draftId, selectedTopicId, content.length() > 50 ? content.substring(0, 50) + "..." : content});
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(EnglishEssayApp.DRAFTS_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 4 && parts[1].equals(draftId)) {
                    JOptionPane.showMessageDialog(this, "Topic: " + getTopicContent(parts[2]) + "\n\nContent:\n" + parts[3], 
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(EnglishEssayApp.DRAFTS_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 4 && parts[1].equals(draftId)) {
                    ...
                    JOptionPane.showMessageDialog(this, "Draft loaded into editor. Topic: " + getTopicContent(parts[2]));
                    updateDraft(draftId);
                    return;
                }
            }
    ...
    private void deleteDraft(JTable draftTable) {
        ...
        String draftId = (String) draftTableModel.getValueAt(row, 0);
        int confirm = JOptionPane.showConfirmDialog(this, "Delete this draft?", "Confirm Delete", JOptionPane.YES_NO_OPTION);
    ...
    private void updateDraft(String draftId) {
        int result = JOptionPane.showConfirmDialog(this, "Save changes to this draft?", "Edit Draft", JOptionPane.OK_CANCEL_OPTION);
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(EnglishEssayApp.GRADED_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 7 && parts[1].equals(essayId)) {
                    ...
                    JOptionPane.showMessageDialog(this, scrollPane, "Essay and Result: " + essayId, JOptionPane.PLAIN_MESSAGE);
                    return;
                }
            }
            JOptionPane.showMessageDialog(this, scrollPane, "Essay: " + essayId + " (Not graded)", JOptionPane.PLAIN_MESSAGE);
        } catch (IOException ex) {
...
class TeacherFrame extends JFrame {
    ...
    public TeacherFrame(String teacherId) {
        ...
        gradedTableModel = new DefaultTableModel(new String[]{"Essay ID", "Student ID", "Topic ID", "Task", "Coherence", "Lexical", "Grammar"}, 0);
        ...
        historyTableModel = new DefaultTableModel(new String[]{"Essay ID", "Student ID", "Topic ID", "Timestamp"}, 0);
    ...
    private void loadGradedEssays() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(EnglishEssayApp.GRADED_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                String[] parts = line.split(" \\| ", 8);
                if (parts.length >= 7) {
                    gradedTableModel.addRow(new Object[]{parts[1], parts[0], parts[2], parts[3], parts[4], parts[5], parts[6]});
                }
            }
        } catch (IOException ex) {
            // File may not exist yet
    ...
    private void gradeEssay(JTable essayTable) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(EnglishEssayApp.GRADED_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                String[] parts = line.split(" \\| ", 8);
                if (parts.length >= 7 && parts[1].equals(essayId)) {
                    JOptionPane.showMessageDialog(this, "This essay has already been graded. Use 'Edit Grade' to modify.");
                    return;
                }
            }
        } catch (IOException ex) {
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            try {
                ...
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(EnglishEssayApp.GRADING_HISTORY_FILE, true))) {
                    String timestamp = String.valueOf(System.currentTimeMillis());
                    writer.write(String.format("%s | %s | %s | %s | %s | %s | %s | %s | %s",
                        teacherId, studentId, essayId, topicId, taskAchievement, coherence, lexical, grammar, comments, timestamp));
                    writer.newLine();
                ...
                if (status.equals("FINAL")) {
                    try (BufferedWriter writer = new BufferedWriter(new FileWriter(EnglishEssayApp.GRADED_FILE, true))) {
                        writer.write(String.format("%s | %s | %s | %s | %s | %s | %s | %s",
                            studentId, essayId, topicId, taskAchievement, coherence, lexical, grammar, comments));
                        writer.newLine();
                        gradedTableModel.addRow(new Object[]{essayId, studentId, topicId, taskAchievement, coherence, lexical, grammar});
                    }
                }
    ...
    private void editGrade(JTable gradedTable) {
        ...
        JTextArea contentArea = new JTextArea("Topic: " + getTopicContent(topicId) + "\n\nContent:\n" + content);
        contentArea.setEditable(false);
        JSlider taskAchievementSlider = new JSlider(JSlider.HORIZONTAL, 1, 9, Integer.parseInt((String) gradedTableModel.getValueAt(row, 3)));
        JSlider coherenceSlider = new JSlider(JSlider.HORIZONTAL, 1, 9, Integer.parseInt((String) gradedTableModel.getValueAt(row, 4)));
        JSlider lexicalSlider = new JSlider(JSlider.HORIZONTAL, 1, 9, Integer.parseInt((String) gradedTableModel.getValueAt(row, 5)));
        JSlider grammarSlider = new JSlider(JSlider.HORIZONTAL, 1, 9, Integer.parseInt((String) gradedTableModel.getValueAt(row, 6)));
        taskAchievementSlider.setMajorTickSpacing(1);
        coherenceSlider.setMajorTickSpacing(1);
        lexicalSlider.setMajorTickSpacing(1);
        grammarSlider.setMajorTickSpacing(1);
    ...
    private void viewGradingHistory(JTable historyTable) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(EnglishEssayApp.GRADING_HISTORY_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                String[] parts = line.split(" \\| ", 9);
                if (parts.length >= 9 && parts[0].equals(teacherId) && parts[2].equals(essayId) && parts[8].equals(timestamp)) {
